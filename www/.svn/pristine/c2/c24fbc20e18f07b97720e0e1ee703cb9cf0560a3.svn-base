...
class MemoryUseNotificationRole
{

    public static function check($serverInfo, $notificationInfo)
    {
        Log::debug("检查主机内存告警", ["hostId" => $serverInfo["hostId"], "result" => ""]);
        ...
        if ($count > 0) {
            try {
                Log::debug("检查主机内存告警", ["hostId" => $server_id, "content" => $content]);
                if ($countType == 1) {
                    $avg_use =  max($used);
                }
                if ($countType == 2) {
                ...
                Log::debug("检查主机内存告警ERROR", ["hostId" => $serverInfo["hostId"], "msg" => $e->getMessage().' at Line '.$e->getLine().' in File '.$e->getFile()]);
            }
        }
    }
}
