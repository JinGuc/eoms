...
install_tools(){
    ...
    if [ $apache_num -gt 0 ] || [ -z $findserverap ] || [ -z $findserverah ];then
        echo
    ...
    if [ $mysql_num -gt 0 ] || [ -z $findserver ];then
        echo
    ...
    if [ $php_num -gt 0 ] || [ -z $findserver ];then
        echo
    ...
    if check_sys packageManager apt; then
        ...
        apt_tools=(xz-utils tar gcc g++ make wget perl curl bzip2 libreadline-dev net-tools cron ca-certificates ntpdate automake)
