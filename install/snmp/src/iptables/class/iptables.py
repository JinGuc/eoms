...
class firewall_main:
    ...
    _rule_path = "/opt/snmp/py-eoms/data/firewall/"
    _ips_path = "/opt/snmp/py-eoms/data/ips.txt"
    _country_path = "/opt/snmp/py-eoms/data/country.txt"
    # 证书验证IP
    _white_list_file = "/opt/snmp/py-eoms/data/whitelist.txt"
    _white_list = []
    ...
    def handle_ufw_country(self, brief, ip_list, types, port_list):
        tmp_path = '/opt/snmp/py-eoms/data/tmp.sh'
        ...
        tmp_file.close()
        public.ExecShell(
            'ipset create ' + brief + ' hash:net; /bin/bash /opt/snmp/py-eoms/data/tmp.sh')
        if port_list:
            for port in port_list:
                public.ExecShell(
